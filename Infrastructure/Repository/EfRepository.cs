...
namespace Infrastructure.Repository
{
  public class EfRepository<T> : IRepository<T> where T : BaseEntity
  {
    ...
    public EfRepository( DbContextOptions<ClientRegisterDBContext> options )
    {
    ...
    {
      // Look in the local tracker to see if the ID is already there
      var local = _context.Set<T>()
          .Local
          .FirstOrDefault( entry => entry.Id.Equals( entity.Id ) );

      // If we found the "old" object being tracked...
      if(local != null)
      {
        // Detach it so EF "forgets" about it
        _context.Entry( local ).State = EntityState.Detached;
      }

      // Now EF is free to track the "new" version of the entity
    ...
    {
      var entityRecord = _dbSet.Find( entity.Id );

      if(entityRecord != null)
      {
        _dbSet.Remove( entityRecord );
      }
